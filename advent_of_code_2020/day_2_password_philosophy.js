// JSON.stringify(document.getElementsByTagName("pre")[0].textContent.split(/\n/));
...
const validPassword = (inputPasswords) => {
  // count the characters by:
  // (1) split the string
  // (2) accumulate the count if element === character
  const countCharacter = (str, char) => {
    return str.split("").reduce((a, x) => (x === char ? ++a : a), 0);
  };

  let validPasswordCount = 0;
  for (let password of inputPasswords) {
    // split the password by the spaces
    let passwordArray = password.split(" ");
    // first element of password array is the range, split it by the hyphen
    let range = passwordArray[0].split('-');
    let minimumRange = range[0];
    let maximumRange = range[1];
    // second element is the required char, delete the :
    let requiredChar = passwordArray[1].split("").shift();
    // third element is the password string
    let passwordString = passwordArray[2];
    const characterCount = countCharacter(passwordString, requiredChar);

    // if the character count is between the range then update password count
    if (characterCount >= minimumRange && characterCount <= maximumRange) {
      validPasswordCount++;
    }
  }
